...
function step()
	...
	for i = 1, num_neighbors do -- for each robot seen
	...
	if(random < count_red/(num_neighbors+1)) then
		MY_COLOR = "red"
	elseif(random < count_red/(num_neighbors+1) + count_green/(num_neighbors+1)) then
		MY_COLOR = "green"
	else
		MY_COLOR = "blue"
	end	
